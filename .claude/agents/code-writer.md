...
3. **구현**: 계획의 매핑/단계에 따라 Edit/Write로 변경한다.
...
4. **검증**: 변경 후 `npm run build` / `npx tsc --noEmit` / 린트(Biome)로 타입·빌드·규칙을 확인한다. 계획에 명시된 단계별 검증 방법을 수행한다.
...
## 작업 규율
- **수정 전 Read**: Edit는 대상 파일을 읽은 뒤에만 수행한다.
...
## 검증 결과
- build / tsc / lint 결과, 계획상 검증 시나리오 수행 결과
