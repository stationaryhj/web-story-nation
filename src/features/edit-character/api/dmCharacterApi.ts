...
import { type IntroBubbleGroup, introBubblesToExampleJson } from '@/lib/utils/storyNationUtil';
  ...
  return useMutation({
    mutationFn: async (formData: SaveMultiImagesPayload): Promise<DmMultiImage[] | null> => {
      ...
      const hasDeleted = formData.multi_images_original.some(
        (orig) =>
          !formData.multi_images.some(
            (item) => item.chrbot_multi_image_key === orig.chrbot_multi_image_key
          )
      );
      ...
      apiFormData.append(
        ...
        formData.world_list_detail_chrbot_key || ''
      );
      apiFormData.append('likeability_yn', formData.likeability_yn?.toString() || '');
      apiFormData.append('image', imageFile);

      const response = await instance.post(
        apiRoute.charbot.inprogress.saveMultiImage,
        apiFormData,
        {
          headers: { 'Content-Type': 'multipart/form-data' },
        }
...
export const useSaveProperty = () => {
  return useMutation({
    mutationFn: async (formData: CharacterFormData) => {
      const firstOpenImages = formData.multi_images.filter(
        (item) => item.lv === 0 || item.lv === 1
      );
      ...
      if (firstOpenImages.length > 0) {
        const images = firstOpenImages.filter(
          (item) => item.default_yn === 1 || item.show_yn === 1
        );
        if (images.length > 0) {
