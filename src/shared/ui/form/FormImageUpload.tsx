    ...
    <div className={wrapperClassName}>
      ...
      {displayUrl ? (
        <div className='relative h-40 w-40'>
          ...
          />
          <div className='absolute bottom-3 right-3 gap-1 rounded-lg bg-overlay/80 px-[7px] py-1 duration-100 hover:bg-overlay'>
            <label
            ...
            >
              ...
              <p className='font-regular text-xs text-text-inverse'>업로드</p>
              <input
                ref={inputRef}
                type='file'
        ...
        <label
          htmlFor='form-image-upload'
          className={cn(
            'flex h-40 w-40 cursor-pointer flex-col items-center justify-center gap-3 rounded-[10px] bg-surface-elevated duration-100 hover:bg-surface-elevated-hover',
            errorMessage && 'border border-danger',
            className
          )}
        >
          <Image src='/images/image_thumbnail.svg' alt='이미지 업로드' width={68} height={70} />
          <p className='text-xs font-medium text-text-muted'>클릭해서 이미지 업로드</p>
          <input
            ref={inputRef}
            type='file'
            id='form-image-upload'
      ...
      {notice && (
        <div className='mt-3'>
          {notice.map((text: string) => (
            <p key={text} className='text-xs text-text-muted'>
              {text}
            </p>
          ))}
        </div>
