...
export class GoogleAuthService extends BaseAuthService {
  ...
  async login(params: LoginParams, callbacks: SocialLoginCallbacks): Promise<LoginResult> {
    try {
      ...
      // 팝업 창 열기
      const popup = this.openPopup(authUrl, 'Google Login');
      ...
      return {
        success: false,
        error: '구글 로그인이 진행 중입니다. 로그인 창에서 인증을 완료해주세요.'
      };
    } catch (error) {
  ...
  async handleCallback(params: CallbackParams): Promise<LoginResult> {
    ...
    const callbackProcessing = localStorage.getItem('google_callback_processing');
    if (callbackProcessing === 'true') {
      console.log('구글 콜백 처리가 이미 진행 중입니다.');
      return {
        success: false,
    ...
    try {
      ...
      // 오류 처리
      if (params.error) {
        throw new Error(`구글 로그인 오류: ${params.error}`);
      }
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : '구글 로그인 콜백 처리 중 오류가 발생했습니다.'
      };
    }
  }
} 