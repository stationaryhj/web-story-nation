...
export class AppleAuthService extends BaseAuthService {
  ...
  async handleCallback(params: CallbackParams): Promise<LoginResult> {
    ...
    try {
      ...
      // 액세스 토큰 요청
      const tokenResponse = await this.getAppleToken(params.code, clientId);

      // Apple 특성상 재인증 시 사용자 정보를 다시 제공하지 않을 수 있음
      // 저장된 정보를 활용하여 보완
      if (tokenResponse.id_token) {
        const jwtData = this.parseJwt(tokenResponse.id_token);
        if (jwtData.sub && (!jwtData.email || !jwtData.name)) {
          const savedUserInfo = localStorage.getItem(`apple_user_${jwtData.sub}`);
          if (savedUserInfo) {
            try {
              const parsedInfo = JSON.parse(savedUserInfo);
              userInfo = { ...jwtData, ...parsedInfo };
            } catch (e) {
