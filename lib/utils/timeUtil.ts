...
export const timeUtil: TimeUtil = {
    ...
    fromNow: (startUnixTimestampMs: number) => {
        ...
        let diffInSeconds = Math.floor((nowMs - startUnixTimestampMs) / 1000); // 현재 시간과 start_date의 차이 (초 단위)
    ...
    timeStampToUTC0: (timeStamp: number): string => {
        let ret = "";
        let d = new Date(timeStamp * 1000); // 타임스탬프를 Date 객체로 변환 (UTC 기준)
    ...
    unixTimeToYMDHM: (value: number): string => {
        ...
        let diff = value - nowTime; // 입력된 시간과 현재 시간의 차이를 ms 단위로 계산합니다.
