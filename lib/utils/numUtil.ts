...
export const numUtil = {
  ...
  convertToBigNumbers: (...values: Array<BigNumber | number | string>) =>
    values.map((value) => numUtil.convertToBigNumber(value)),

  /** 모든 인자를 BigNumber로 변환 후 최소값을 찾음 */
  minBigNumber: (...values: Array<BigNumber | number | string>) =>
    numUtil
  ...
  maxBigNumber: (...values: Array<BigNumber | number | string>) =>
    numUtil
      .convertToBigNumbers(...values)
      .reduce((max, current) =>
  ...
  randDiceObj: (obj: { [key: string]: BigNumber | number | string }) => {
    ...
    const randomValue = numUtil.randInt(1, totalWeight);
  ...
  simpleText: (address: string, front: number, end: number) => {
    if (address && address.length > 5) {
      const frontStr = address.substr(0, front);
      const endStr = address.substr(address.length - end, address.length);
