...
interface MainStoreData {
    ...
    UpdateEtcCharactersPaging: (module_id: number, ranking_type: number, page: number, pageSize: number) => Promise<Character[]>;
    ClearEtcCharactersSlide: () => void;
    ClearLatestCharactersSlide: () => void;
    UpdateharactersPaging: (beforeDatas: Character[], module_id: number, ranking_type: number, page: number, pageSize: number) => Promise<Character[]>;
}
...
export const useRecommendSectionStoreData = create<MainStoreData>((set, get) => ({
  ...
  UpdateharactersPaging: async (beforeDatas: Character[], module_id: number, ranking_type: number, page: number, pageSize: number) => {
    try {
      const data = await queryClient.fetchQuery({
