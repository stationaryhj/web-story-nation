...
export const useCreateCharacterData = create<CreateCharacterStore>((set, get) => ({
  ...
  // 대화 예시 함수들
  addConversationExample: () => set((state) => {
    // 최대 3개까지만 추가 가능
  ...
  updateConversationExample: (id, text) => set((state) => ({
    formData: {
      ...state.formData,
  ...
  removeConversationExample: (id) => set((state) => ({
    formData: {
      ...state.formData,
  ...
  setConversationExampleEditMode: (id, isEditing) => set((state) => ({
    formData: {
      ...state.formData,
      conversationExamples: state.formData.conversationExamples.map(ex =>
        ex.id === id ? { ...ex, isEditing } : ex
      ),
    },
  })),
  
  setConversationExampleVisibility: (id, visibility) => set((state) => ({
    formData: {
      ...state.formData,
      conversationExamples: state.formData.conversationExamples.map(ex =>
  ...
  saveInProgress: async (finishYn = 0) => {
    try {
      ...
      const payload = {
        ...
        // 대화 예시 (공개=1, 비공개=0)
        example_show_yn: formData.conversationExamples?.[0]?.visibility === 'public' ? 1 : 0,
        finish_yn: finishYn,
      }
