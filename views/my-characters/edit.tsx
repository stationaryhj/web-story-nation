...
} from '@/store/useCreateCharacterData';
...
export default function EditCharacterPage() {
  ...
  const privateOpenCharacterCount =
    inProgressData?.chrbotList.data.filter((char) => char.finish_yn === 1 && char.show_yn === 0)
      .length || 0;
  ...
  if (storeError || error) {
    return (
      <div className='flex min-h-screen items-center justify-center bg-surface'>
        <div className='text-danger'>{error || '데이터를 불러오는데 실패했습니다.'}</div>
      </div>
    );
  }
    ...
    <div className='min-h-screen bg-surface pb-20'>
      <SectionTransition>
        <div className='container mx-auto px-4 py-8'>
          <div className='overflow-hidden rounded-xl bg-surface-elevated shadow-sm'>
            {/* 상단 탭 네비게이션 */}
            <div className='flex border-b border-border-default' style={{ wordBreak: 'keep-all' }}>
              <button
                type='button'
                className={`flex-1 px-2 py-4 text-center ${
                  activeTab === 'basic' ? 'bg-brand/10 font-medium text-brand' : 'text-text-muted'
                }`}
              ...
              <button
                type='button'
                className={`flex-1 px-2 py-4 text-center ${
                  activeTab === 'detail' ? 'bg-brand/10 font-medium text-brand' : 'text-text-muted'
                }`}
              ...
              <button
                className={`flex-1 px-2 py-4 text-center ${
                  activeTab === 'image' ? 'bg-brand/10 font-medium text-brand' : 'text-text-muted'
                }`}
              ...
              <button
                type='button'
                className={`flex-1 px-2 py-4 text-center ${
                  activeTab === 'last' ? 'bg-brand/10 font-medium text-brand' : 'text-text-muted'
                }`}
            ...
            {/* 하단 버튼 */}
            <div className='flex justify-between border-t border-border-default p-6'>
              <button
                type='button'
                onClick={() => router.back()}
                className='rounded-lg border border-border-default bg-surface-elevated px-6 py-3 text-xs text-text-primary transition-colors hover:bg-surface-elevated-hover sm:text-base'
                disabled={isSaving}
              >
                취소
              </button>
              <button
                ...
                className={`rounded-lg px-6 py-3 transition-colors ${
                  !isSaving
                    ? 'bg-brand text-xs text-text-inverse hover:bg-brand-hover sm:text-base'
                    : 'cursor-not-allowed bg-brand/50 text-text-inverse'
                }`}
              >
