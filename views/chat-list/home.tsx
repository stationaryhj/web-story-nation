...
import {
  bridgeCharbotChatDataToChatList,
  getChatRoomEncryptData,
  rijndaelEncrypt,
} from '@/lib/utils/storyNationUtil';
import { chatApi, contentApi, createApi } from '@/services/api/storyNationApi';
...
const Toast = ({ message, isVisible, onClose }: ToastProps) => {
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          className='fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-overlay/90 text-text-inverse px-4 py-3 rounded-lg shadow-lg z-50 flex items-center'
        ...
        >
          ...
          <button
            onClick={onClose}
            className='p-1 rounded-full hover:bg-overlay/70 transition-colors'
          >
          ...
          <div className='container mx-auto px-4 py-6'>
            <div className='flex flex-col'>
              ...
              >
                ...
                <div className='flex md:flex-row pb-4 mb-4 justify-between border-b border-border-default'>
                    ...
                    <div className='space-y-3'>
                      {filteredChatList.length > 0 ? (
                        filteredChatList.map((chat) => (
                          <motion.div
                            key={chat.id}
                            className='flex items-center p-3 rounded-lg hover:bg-surface-elevated-hover cursor-pointer group'
                          ...
                          >
                            ...
                            <div className='grid grid-cols-2 gap-2 ml-3 transition-opacity'>
                              <button
                                onClick={(e) =>
                                  handleTogglePin(e, Number(chat.id), Number(chat.fixed))
                                }
                                className={`p-2 rounded-full hover:bg-surface-elevated-hover transition-colors ${
                                  Number(chat.fixed) === 1
                              ...
                              <button
                                onClick={(e) =>
                                  handleDeleteClick(e, { id: Number(chat.id), name: chat.name })
                                }
                                className='p-2 rounded-full hover:bg-danger/10 text-danger hover:text-danger transition-colors'
                              >
