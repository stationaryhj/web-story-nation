...
export default function ShopRecharge() {
  ...
  const {
    data: accountData,
    isLogin,
    initFromExternalToken,
  } = useAccountStore((state) => ({
    data: state.data,
    isLogin: state.isLogin,
    initFromExternalToken: state.initFromExternalToken,
  }));
  ...
  const coinChargeUseHistoryDataList =
    (coinChargeUseHistoryData?.historyList?.data as UseHistoryData[]) || [];
                              ...
                              {(() => {
                                try {
                                  ...
                                  return (
                                    <span
                                      className={`px-2 py-1 rounded-full text-xs font-medium ${
                                        transaction.property === '충전' ||
                                        transaction.property === '보너스'
                                          ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400'
                                          : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'
                                      }`}
                                    >
