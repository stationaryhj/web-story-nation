  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            className={`relative rounded-lg bg-surface-elevated shadow-xl ${
          ...
          >
            ...
            {(title || showCloseButton) && (
              <div
                className={`flex items-center justify-between border-b border-border-default p-4 ${headerClassName}`}
              >
                {title && <h3 className='text-lg font-medium text-text-primary'>{title}</h3>}
                {showCloseButton && (
                  <button
                    type='button'
                    className='ml-auto flex h-8 w-8 items-center justify-center rounded-full text-text-muted transition-colors hover:bg-surface-elevated-hover hover:text-text-primary'
                    onClick={onClose}
                    aria-label='닫기'
                  >
            ...
            {/* 푸터 */}
            {footer && (
              <div className={`border-t border-border-default p-4 ${footerClassName}`}>
                {footer}
              </div>
            )}
          </motion.div>
