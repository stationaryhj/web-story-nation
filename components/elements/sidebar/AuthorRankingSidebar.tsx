...
export default function AuthorRankingSidebar({
  isOpen,
  onClose,
  isSidebar = false,
}: AuthorRankingSidebarProps) {
  ...
  const renderSkeletons = () => {
    return Array(5)
      .fill(0)
      .map((_, index) => (
        <div key={`skeleton-${index}`} className='border border-border-default rounded-lg mb-4 p-4'>
          <div className='flex items-center space-x-4'>
            <div className='w-8 h-8 rounded-md bg-surface-elevated animate-pulse'></div>
            <div className='w-16 h-16 rounded-full bg-surface-elevated animate-pulse'></div>
            <div className='flex-1'>
              <div className='h-4 w-24 bg-surface-elevated rounded animate-pulse mb-2'></div>
              <div className='h-3 bg-surface-elevated rounded animate-pulse w-full'></div>
            </div>
          </div>
        </div>
      ));
    ...
    return rankingData.map((author, index) => {
      ...
      if (index < 20) {
        return (
          <div
            key={author.id}
            className='border border-border-default rounded-lg mb-4 p-4 cursor-pointer hover:bg-surface-elevated transition-colors'
          ...
          >
            <div className='flex items-center'>
              ...
              <div className='relative flex-shrink-0'>
                {/* 프로필 이미지 */}
                <div className='relative w-16 h-16 rounded-full overflow-hidden bg-surface-elevated'>
                ...
                <div
                  className={`absolute -top-2 -left-2 w-7 h-7 ${getRankBgColor(index + 1)} text-text-inverse flex items-center justify-center font-bold rounded-full shadow-md z-10`}
                >
                  {index + 1}
                </div>
              </div>
              ...
              <div className='ml-4 flex-1 overflow-hidden'>
                ...
                <p className='text-xs text-text-muted mt-1 line-clamp-2'>{author.description}</p>
              </div>
            </div>
          </div>
        );
      ...
      return (
        <div
          key={author.id}
          className='flex items-center px-4 py-2 cursor-pointer hover:bg-surface-elevated transition-colors rounded-lg'
          onClick={() => handleAuthorClick(author)}
        >
          <div
            className={`w-6 h-6 ${getRankBgColor(index + 1)} text-text-inverse flex items-center justify-center font-medium rounded-md text-sm mr-3`}
          >
            {index + 1}
          </div>
  ...
  return (
    <BaseSidebar isOpen={isOpen} onClose={onClose} title='작가 랭킹'>
      {/* 필터 영역 */}
      <div className='px-6 py-4 border-b border-border-default space-y-4'>
        {/* 탭 */}
        <div>
