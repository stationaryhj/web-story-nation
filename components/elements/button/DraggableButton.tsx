...
import { Lightbulb, Minus, Pen, Plus, X } from 'lucide-react';
...
export default function DraggableButton({
  ...
  return (
    <>
      ...
      >
        <div className='transform scale-125 flex items-center justify-center w-full h-full'>
          {buttonContent}
        </div>
      </button>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <>
            {/* 메뉴 배경 (클릭 시 메뉴 닫힘) */}
            <motion.div
              className='fixed inset-0 bg-overlay/10'
              style={{ zIndex: 998 }}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            {menuButtons.map((button) => (
              <motion.button
                key={button.id}
                className={`fixed rounded-full ${button.color} text-text-inverse flex items-center gap-2 shadow-lg px-4`}
                style={{
