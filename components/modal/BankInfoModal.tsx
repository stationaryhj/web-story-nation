...
interface BankInfoModalProps {
  ...
  onBankInfoChange: (bankInfo: { bank: string; accountNumber: string; accountHolder: string }) => void
}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="relative" ref={bankDropdownRef}>
          ...
          {showBankList && (
            <div className="absolute mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
              {isLoading ? (
                <div className="px-4 py-2 text-gray-500">은행 목록을 불러오는 중...</div>
              ) : error ? (
                <div className="px-4 py-2 text-red-500">은행 목록을 불러오지 못했습니다</div>
              ) : (
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            예금주 <span className="text-xs text-gray-500">(한글, 영문 가능)</span>
          </label>
          <input
            type="text"
            value={accountHolder}
        ...
        <div className="bg-red-50 p-3 rounded-lg mt-2">
          <p className="text-sm text-red-600 font-medium leading-tight">
            입력하신 계좌번호와 예금주를 정확히 확인해 주세요. 잘못된 정보 입력으로 인한 문제는 책임지지 않습니다.
          </p>
        </div>
      </div>
    </BaseModal>
