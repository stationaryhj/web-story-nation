...
export default function CharactorModal({ isOpen, onClose }: CharactorModalProps) {
  ...
  return (
    ...
    >
      {/* 모달 헤더 */}
      <div className='sticky top-0 z-[102] border-b border-border-default bg-surface-elevated'>
        <div className='flex items-center justify-between px-4 py-4'>
          <h1 className='text-md font-bold text-text-primary md:text-xl'>
          ...
          <div className='flex items-center gap-2'>
            {variant === 'default' ? (
              <button
                ...
                className='flex h-7 w-7 items-center justify-center rounded-full bg-surface-elevated text-text-muted transition-colors hover:bg-surface-elevated-hover md:h-9 md:w-9'
              >
      ...
      <div className='flex h-[calc(90vh-88px)] flex-col overflow-hidden md:flex-row'>
          ...
          <div className='flex flex-col items-start'>
            ...
            <div className='relative mb-6 flex w-full items-center justify-center'>
              {selectedCharacter.imageUrl && (
                <>
                  ...
                  <div
                    id='image_container'
                    className='relative flex aspect-[3/4] w-full items-center justify-center overflow-hidden rounded-xl bg-surface-elevated'
                  >
                      ...
                      <div className='absolute bottom-2 left-2 z-30 flex items-center justify-center gap-4'>
                        {selectedCharacter.multi_image_count > 0 && (
                          <button
                            ...
                            className='rounded-full bg-overlay/50 px-2 py-1 text-xs text-text-inverse'
                          >
                        ...
                        {selectedCharacter.likeability_yn === 1 && (
                          <div className='flex items-center justify-center gap-2 rounded-full bg-overlay/50 px-2 py-1 text-xs text-text-inverse'>
                            <Image
            ...
            <div className='mb-6 flex w-full items-center justify-between'>
              ...
              <div className='flex cursor-pointer items-center' onClick={handleSelectCreator}>
                {/* 작가 섬네일 */}
                <div className='mr-2 flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-surface-elevated'>
                  <Image
              ...
              <div className='flex items-end gap-6'>
                ...
                >
                  <FontAwesomeIcon icon={faHeart} className='h-5 w-5 text-red-500' />
                  <span className='text-text-muted'>{selectedCharacter.likeCount || 0}</span>
                </div>
                <div className='flex items-center space-x-2'>
                  <FontAwesomeIcon icon={faMessage} className='h-5 w-5 text-brand' />
                  <span className='text-text-muted'>{selectedCharacter.commentCount || 0}</span>
                </div>
              </div>
            </div>
            ...
            {/* 간략한 캐릭터 설명 */}
            <div className='mt-4 w-full rounded-lg border border-border-default bg-gradient-to-r from-brand/10 to-surface-elevated'>
              <div className='p-4'>
                <h3 className='mb-3 flex items-center text-base font-semibold text-text-primary'>
        ...
        <div className={`${isMobile ? 'hidden' : 'relative flex h-full flex-col md:w-[60%]'}`}>
          ...
          <div className='overflow-y-auto pb-20'>
            <div className='p-5'>
              {/* 첫 번째 섹션: 캐릭터 소개 */}
              {isContentShow == 1 && (
                <div className='mb-4 rounded-lg border border-border-default bg-surface-elevated p-5 shadow-sm'>
                ...
                <div className='mb-4 rounded-lg border border-border-default bg-surface-elevated p-5 shadow-sm'>
                  ...
                  <div className='space-y-4'>
                    {exampleDatas.map((data: exampleDatas, index: number) => (
                      ...
                      >
                        <h4 className='mb-2 text-sm font-medium text-text-primary'>{data.title}</h4>
                        <div className='space-y-2'>
                          <div className='flex items-start space-x-2'>
                            <div className='flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-surface-elevated text-text-muted'>
                              <FontAwesomeIcon icon={faUser} className='h-3 w-3' />
                            </div>
                            <div className='overflow-hidden whitespace-pre-wrap whitespace-pre-wrap break-words break-words rounded-lg bg-surface-elevated p-2 text-sm'>
                              {getChangeNameTag(data.characterMsg, selectedCharacter.name)}
                            </div>
                          </div>
                          <div className='flex justify-end space-x-2'>
                            <div className='overflow-hidden whitespace-pre-wrap break-words rounded-lg bg-brand/10 p-2 text-start text-sm'>
              ...
              <div className='rounded-lg border border-border-default bg-surface-elevated p-5 shadow-sm'>
                <h3 className='flex items-center text-lg font-semibold text-text-primary'>
                  <span className='mr-2 inline-block h-5 w-1.5 rounded-full bg-primary-500'></span>
                  첫 메시지
                </h3>
                <div className='flex flex-col justify-between'>
                  <div className='flex flex-col rounded-lg bg-surface/80 p-4'>
                    {isDmMode ? (
                            ...
                            >
                              {group.speaker === 'character' && (
                                <div className='relative mr-3 flex-shrink-0'>
                                  <div className='h-10 w-10 overflow-hidden rounded-full border-2 border-brand/30 shadow-sm'>
                                    {selectedCharacter.imageUrl ? (
                                      <Image
                                        src={selectedCharacter.imageUrl}
                                  ...
                                  {group.messages.map((m) => {
                                    ...
                                    if (isImageToken) {
                                      return imageUrl ? (
                                        <div
                                          key={m.id}
                                          className='h-[160px] w-[160px] overflow-hidden rounded-lg'
                                        >
                                          <Image
                                            src={imageUrl}
                                            alt='인트로 이미지'
                                        ...
                                        <div
                                          key={m.id}
                                          className='h-[160px] w-[160px] animate-pulse rounded-lg bg-surface-elevated'
                                        />
                                      );
                                    }
                                    if (isVoice) {
                                      return (
                                        <VoiceBubble
                                          ...
                                          className='rounded-lg rounded-tl-none border border-brand/20 bg-brand/10 text-text-primary'
                                        />
                                      );
                                    }
                                    return (
                                      <div
                                        ...
                                        className={`rounded-lg p-3 text-sm shadow-sm ${
                                          group.speaker === 'character'
                                            ? 'rounded-tl-none border border-brand/20 bg-brand/10 text-text-primary'
                                            : 'rounded-tr-none border border-border-default bg-surface-elevated text-text-primary'
                                        }`}
                                      >
                      ...
                      <div className='flex items-start'>
                        <div className='relative mr-3 flex-shrink-0'>
                          ...
                          <div className='absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-surface bg-green-500'></div>
                        </div>
                        <div className='relative max-w-[85%]'>
                          <div className='mb-1 text-xs font-medium text-brand'>
                            {selectedCharacter.name || '캐릭터'}
                          </div>
                          <div className='rounded-lg rounded-tl-none border border-brand/20 bg-brand/10 p-3 text-text-primary shadow-sm'>
          ...
          {/* 대화 시작 버튼 - 항상 하단에 고정 */}
          <div className='absolute bottom-0 left-0 right-0 w-full bg-surface-elevated p-4'>
            <button
              onClick={handleStartChat}
              className='flex w-full items-center justify-center rounded-lg bg-brand px-6 py-4 font-medium text-text-inverse transition-all hover:bg-brand-hover'
            >
        ...
        <div className='relative h-full w-full md:hidden'>
          <div className='absolute inset-0 flex flex-col'>
                        ...
                        >
                          ...
                          <div className='space-y-2'>
                            <div className='flex items-start space-x-2'>
                              ...
                              <div className='whitespace-pre-wrap break-words rounded-lg bg-surface-elevated p-2 text-sm'>
                                {getChangeNameTag(data.characterMsg, selectedCharacter.name)}
                              </div>
                            </div>
                            <div className='flex items-start justify-end space-x-2'>
                              <div className='whitespace-pre-wrap break-words rounded-lg bg-brand/10 p-2 text-end text-sm'>
                                {getChangeNameTag(data.userMsg, selectedCharacter.name)}
                              </div>
                            </div>
                          </div>
            ...
            {/* 대화 시작 버튼 - 모바일 */}
            <div className='absolute bottom-0 left-0 right-0 z-[103] border-t border-border-default bg-surface-elevated p-2'>
              <button
                onClick={handleStartChat}
                className='flex w-full items-center justify-center rounded-lg bg-brand px-6 py-3.5 font-medium text-text-inverse shadow-sm transition-colors hover:bg-brand-hover'
              >
                <FontAwesomeIcon icon={faComment} className='mr-2' />
                대화 시작하기
              </button>
