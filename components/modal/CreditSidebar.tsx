...
export default function CreditSidebar({}: CreditSidebarProps) {
  ...
  const sidebarContent = (
    <>
      <motion.div
        className="fixed inset-0 bg-black/30 dark:bg-black/60 z-[999]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        onClick={closeModal}
      />
      <motion.div
        className="fixed top-0 right-0 w-80 sm:w-96 h-full bg-white dark:bg-dark-background-light z-[999] shadow-xl"
        initial={{ x: '100%' }}
        animate={{ x: 0 }}
        exit={{ x: '100%' }}
