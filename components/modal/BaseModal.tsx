      ...
      {isOpen && (
        <Portal>
          <div className={`fixed inset-0 z-[1100] flex min-w-[355px] ${positionClasses[position]}`}>
            ...
            <motion.div
              {...getAnimationProps()}
              className={`relative z-[1102] rounded-xl px-2 bg-surface-elevated shadow-lg ${sizeClasses[size]} ${className} overflow-hidden flex flex-col`}
            ...
            >
              ...
              {!hideHeader && showCloseButton && (
                <div
                  className={`relative flex justify-end p-3 pb-0 ${headerClassName} flex-shrink-0`}
                >
                  <button
                    onClick={onClose}
              ...
              {/* 모달 내용 */}
              <div className={`${bodyClassName} flex-grow overflow-y-auto`}>
                {/* 아이콘이 있는 경우 타이틀 위에 표시 */}
                {isIcon && icon && <div className='mb-3 flex justify-center'>{icon}</div>}
                {/* 타이틀을 바디에 포함 (가운데 정렬) */}
                {title && (
                  <h2 className='mb-4 text-center text-xl font-bold text-text-primary'>{title}</h2>
                )}
                {children}
              </div>
              ...
              {footerContent && (
                <div
                  className={`border-t border-border-default p-4 ${footerClassName} flex-shrink-0`}
                >
                  {footerContent}
                </div>
              )}
