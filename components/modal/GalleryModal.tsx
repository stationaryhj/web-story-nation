...
import { faTimes, faLock, faCheck, faThumbtack, faExpand } from '@fortawesome/free-solid-svg-icons'
...
export default function GalleryModal({ isOpen, onClose }: GalleryModalProps) {
  ...
  const { selectedMultiImageData, multiImages, openImageCount,
    unlockMultiImage, fetchMultiImageData, chatImageSave } = useMultiImageStore()
  ...
  const handleUnlockMultiImage = async () => {
    ...
    if(result !== null && result !== 0) {
      ...
      // 보상 이미지 설정
      const rewardImgUrl = multiImages.find(item => item.chrbot_multi_image_key === selectedMultiImageKey)?.img_url || ''
  ...
  const handleSelectedHoldingBgImage = async (chrbot_multi_image_key: number, img_fixed?: number | 0) => {
  ...
  // 레벨별로 그룹핑
  const groupedByLevel = multiImages.reduce((acc, item) => {
    const level = item.lv
    if (!acc[level]) {
      acc[level] = []
    }
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-[103] bg-white dark:bg-dark-background-light flex items-center justify-between py-4 px-4 border-b border-secondary-100 dark:border-dark-secondary-800">
      ...
      <div className="h-full overflow-y-auto z-[102] bg-white dark:bg-dark-background-light">
            ...
            <div key={level} className='space-y-2'>
              ...
              {level > 0 &&
                <div className='flex items-center gap-2'>
                  <h3 className='text-sm font-semibold text-secondary-700 dark:text-dark-secondary-300'>
                    lv.{level}
                  </h3>
                  <div className='flex-1 h-px bg-secondary-200 dark:bg-dark-secondary-700'></div>
                </div>
              }
              ...
              <div className='grid grid-cols-3 md:grid-cols-5 gap-2'>
                {groupedByLevel[level].map((item, index) => (
                  <div key={`${level}-${index}`} className='w-full h-full bg-secondary-100 dark:bg-dark-secondary-800 rounded-lg aspect-[3/4]'>
                    {item.default_yn !== 1 && item.show_yn !== 1 ? (
                      <div className='w-full h-full bg-secondary-100 dark:bg-dark-secondary-800 rounded-lg aspect-[3/4] flex items-center justify-center relative'>
                        {/* <Image
                        ...
                        <button className='absolute bottom-0 left-0 w-full h-full text-secondary-500 dark:text-dark-secondary-400 text-xs' onClick={() => handleOpenImage(item.chrbot_multi_image_key)}>
                          <div className='text-black p-1 rounded-lg'>
                          <FontAwesomeIcon
                            icon={faLock} 
                            className='h-14 w-14 md:h-14 md:w-14 text-black drop-shadow-2xl bg-gray-500/50 rounded-full p-2'
                          />
                          </div>
                        </button>
                      </div>
                      ...
                      <div className='w-full h-full relative'>
                        ...
                        <div className='absolute top-2 left-2'>
                          <button
                            ...
                            className='w-6 h-6 rounded-full bg-white dark:bg-dark-secondary-800 text-secondary-500 dark:text-dark-secondary-400 hover:bg-secondary-200 dark:hover:bg-dark-secondary-700 transition-colors flex items-center justify-center'>
                        ...
                        {/* {bgImageKey === item.chrbot_multi_image_key && ( */}
                          <div className='absolute top-2 right-2'>
                            <button
                              onClick={() => handleSelectedHoldingBgImage(item.chrbot_multi_image_key, 1)}
                              className='w-6 h-6 rounded-full bg-white dark:bg-dark-secondary-800 text-secondary-500 dark:text-dark-secondary-400 hover:bg-secondary-200 dark:hover:bg-dark-secondary-700 transition-colors flex items-center justify-center'>
      ...
      />
      <ConfirmActionModal isOpen={confirmActionOpen} onClose={() => setConfirmActionOpen(false)} title="확인" description={unLockDesc} onConfirm={handleUnlockMultiImage} />
