  ...
  if (submitted) {
    return (
      <div className='p-8 text-center'>
        ...
        <h3 className='text-xl font-semibold text-text-primary mb-2'>신고가 접수되었습니다</h3>
        <p className='text-text-muted'>빠른 시일 내에 검토 후 적절한 조치를 취하겠습니다.</p>
      </div>
    );
  }
      ...
      <div className='p-6'>
        ...
        <div className='mb-6'>
          <h3 className='font-medium text-text-primary mb-3 flex items-center'>
            <span className='inline-block w-5 h-5 bg-blue-500 rounded-full text-text-inverse flex items-center justify-center text-xs mr-2'>
              1
            </span>
            사유 선택
          </h3>
                ...
                {reason.id === selectedReasonId && (
                  <div className='p-4 bg-surface-elevated'>
                    ...
                    <textarea
                      ...
                      className='w-full border border-border-default rounded-lg p-3 text-sm bg-surface text-text-primary min-h-[120px] focus:border-blue-400 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-900/50 outline-none transition-all'
                      maxLength={100}
                    />
                    <div className='flex justify-end mt-2'>
                      <span
                        className={`text-xs ${description.length >= 90 ? 'text-danger' : 'text-text-muted'}`}
                      >
        ...
        <button
          ...
          className={`px-8 py-3 rounded-lg font-medium transition-all ${
            selectedReasonId !== null
              ? 'bg-danger hover:bg-danger/90 active:bg-danger/80 text-text-inverse shadow-sm hover:shadow'
