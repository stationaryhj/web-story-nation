...
interface CharactorOpenModalProps {
  isOpen: boolean
  chatBotKey: string
  onClose: () => void
}

export default function CharactorOpenModal({ isOpen, onClose, chatBotKey }: CharactorOpenModalProps) {
  ...
  const {
    ...
    error: chatBotError,
    refetch,
  } = ReqGetChatBot(Number(chatBotKey))
  ...
  const content = getChangeNameTag(chatBotData?.chrbot?.content || '', chatBotData?.chrbot?.title || '')
  const isContentShow = chatBotData?.chrbot?.content_show_yn === 1 && content !== '' ? 1 : 0

  // 모달 열릴 때 이미지 미리 로딩
  useEffect(() => {
    if (chatBotData && chatBotData?.chrbot) {
      const img = new window.Image()
      img.src = chatBotData?.chrbot?.img_url
  ...
  const handleStartChat = async () => {
    if (chatBotData && chatBotData?.chrbot.world_list_detail_chrbot_key) {
      ...
      if (!isAdult() && chatBotData?.chrbot.nsfw === 1) {
        openModal('adultVerification')
        return
      }

      handleClose()
      const chatId = String(chatBotData?.chrbot.world_list_detail_chrbot_key).trim()
  ...
  const handleLike = async () => {
    const response = await contentApi.CharBotLike(Number(chatBotData?.chrbot.world_list_detail_chrbot_key))
  ...
  const handleReportSubmit = async (reason: number | null, description: string) => {
    try {
      ...
      const response = await contentApi.ReportChatBot(10, Number(chatBotData?.chrbot.world_list_detail_chrbot_key), reason || 0, description, 'KR')
  ...
  const handleCreateCharacter = async () => {
    router.push(`/my-characters/edit/${chatBotData?.chrbot.world_list_detail_chrbot_key}`)
    onClose()
  }

  const handleSelectCreator = () => {
    router.push(`/author/${chatBotData?.chrbot.nick_nm}`)
    onClose()
  }
                  ...
                  >
                    <div className="relative w-full h-full">
                      <Image
                        ...
                        alt={chatBotData?.chrbot.img_url || '캐릭터 이미지'}
                        priority
                        fill
                        ...
                        <div className="relative flex-shrink-0 mr-3">
                          ...
                          <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-primary-200 dark:border-primary-800 shadow-sm">
                            {chatBotData?.chrbot.img_url ? (
                              <Image
                                src={getImageUri(chatBotData?.chrbot.img_url)}
                                alt={chatBotData?.chrbot.title || '캐릭터'}
                                width={40}
                                height={40}
                        ...
                        <div className="relative max-w-[85%]">
                          ...
                          <div className="bg-primary-50 dark:bg-primary-900/30 text-secondary-800 dark:text-secondary-200 p-3 rounded-lg rounded-tl-none shadow-sm border border-primary-100 dark:border-primary-800/50">
                            <p className="text-sm leading-relaxed whitespace-pre-wrap break-words">
                              {getChangeNameTag(chatBotData?.chrbot.first_talk || '', chatBotData?.chrbot.title)}
                            </p>
                          </div>
                        </div>
                      </div>
          ...
          <div className="absolute inset-0 flex flex-col">
            <div className="flex-1 overflow-y-auto pb-20">
              <div className="p-5 space-y-4">
                ...
                {/* 해시태그 */}
                <div className="flex flex-wrap justify-start gap-2 mb-4">
                  {chatBotData?.chrbot.tags?.split(',').slice(0, 7).map((tag: string, index: number) => (
                    <span
                      key={`tag-${index}`}
                      className="rounded-full bg-primary-50 px-3 py-1 text-sm text-primary-700 dark:bg-dark-primary-900 dark:text-dark-primary-300"
                    >
